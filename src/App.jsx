...
const Sidebar = () => {
  ...
  const navItems = [
    ...
    { path: '/ai-doll', label: 'AI Doll', icon: '🤖' },
  ];
    ...
    <div className="sidebar">
      ...
      <div className="user-profile-mini">
        ...
        <div className="user-info">
          ...
          <p className="user-status">Pro Student</p>
        </div>
      </div>
    </div>
  );
...
const Layout = ({ children }) => {
  const location = useLocation();
  const isNoLayoutPage = location.pathname === '/onboarding';
...
const App = () => {
  ...
  return (
    <Router>
      <Layout>
        <Routes>
          <Route path="/" element={<Navigate to={profile.hasOnboarded ? "/dashboard" : "/onboarding"} />} />
          <Route 
            path="/onboarding" 
